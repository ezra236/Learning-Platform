...
from .models import (
    PrepExam, PrepQuestion, PrepAttempt, PrepReport, PrepBookmark
)
from .serializers import (
    PrepExamSerializer as PrepExamDetailSerializer,
    PrepQuestionSerializer as PrepQuestionDetailSerializer,
    PrepAttemptSerializer, PrepReportSerializer, PrepBookmarkSerializer
)
from .permissions import IsRegularUser  # keep your permission helper as before

# Endpoint: /api/prep/exams/by-name/?name=...
class PrepExamByNameAPIView(generics.RetrieveAPIView):
    permission_classes = [IsAuthenticated, IsRegularUser]
    serializer_class = PrepExamDetailSerializer

    def get_object(self):
        name = self.request.query_params.get('name')
        if not name:
            raise Http404
        return get_object_or_404(PrepExam, name=name)


# Check if an attempt exists for this user & exam
# GET /api/prep/attempts/check/<int:exam_id>/
class PrepCheckAttemptAPIView(APIView):
    permission_classes = [IsAuthenticated, IsRegularUser]

    def get(self, request, exam_id):
        attempt = PrepAttempt.objects.filter(user=request.user, exam_id=exam_id).first()
        if not attempt:
            return Response({"exists": False}, status=status.HTTP_200_OK)
        return Response({"exists": True, "attempt": PrepAttemptSerializer(attempt).data}, status=status.HTTP_200_OK)


class PrepAttemptCreateAPIView(generics.CreateAPIView):
    permission_classes = [IsAuthenticated, IsRegularUser]
    serializer_class = PrepAttemptSerializer

    def perform_create(self, serializer):
        # Ensure uniqueness: if an attempt exists, return it instead (avoid duplicates)
        exam = serializer.validated_data.get('exam')
        existing = PrepAttempt.objects.filter(user=self.request.user, exam=exam).first()
        if existing:
...
class PrepAttemptDetailAPIView(generics.RetrieveUpdateDestroyAPIView):
    permission_classes = [IsAuthenticated, IsRegularUser]
    serializer_class = PrepAttemptSerializer
    queryset = PrepAttempt.objects.all()

    def get_queryset(self):
        return PrepAttempt.objects.filter(user=self.request.user)


class PrepSubmitAttemptAPIView(APIView):
    permission_classes = [IsAuthenticated, IsRegularUser]

    def post(self, request, pk):
        attempt = get_object_or_404(PrepAttempt, pk=pk, user=request.user)
        ...
        return Response({
            ...
            "score": score,
            "total": total,
            "attempt": PrepAttemptSerializer(attempt).data
        }, status=status.HTTP_200_OK)


class PrepReportCreateAPIView(generics.CreateAPIView):
    permission_classes = [IsAuthenticated, IsRegularUser]
    serializer_class = PrepReportSerializer

    def perform_create(self, serializer):
        serializer.save(user=self.request.user)


class PrepBookmarkCreateAPIView(generics.CreateAPIView):
    permission_classes = [IsAuthenticated, IsRegularUser]
    serializer_class = PrepBookmarkSerializer
