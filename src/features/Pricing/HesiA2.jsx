...
export default function Hesia2t() {
  const [plans, setPlans] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  // per-plan button loading state: { [planId]: true/false }
  ...
  useEffect(() => {
    ...
    async function fetchPlans() {
      try {
        ...
        const plansRes = await fetch(`${BASE}/api/plans/hesia2/`, {
          method: "GET",
          headers: {
  ...
  if (!plans || plans.length === 0) {
    return (
      <div className={styles.emptyState}>
        ...
        <p className={styles.emptyMessage}>Check back later for new ATI TEAS 7 plans!</p>
      </div>
    );
  }
  ...
  const handleGetStarted = (plan) => {
    // mark this plan's button as loading
    setBtnLoading(prev => ({ ...prev, [plan.id]: true }));

    // navigate to target signup page (immediate)
    // If you want to pass plan id or title, you can append query params here.
    // e.g. const url = `/rushhour/hesi/signup?planId=${plan.id}`;
    const url = "/user/signup";

    if (typeof window !== "undefined") {
      // Use window.location to navigate (works across Next.js versions)
      window.location.href = url;
    } else {
      // Fallback - shouldn't normally happen in a client component
      // (keeps the loading state until navigation is possible)
      console.warn("Navigation attempted in non-window environment:", url);
    }
  };
    ...
    <div className={styles.wrapper}>
      <div className={styles.container}>
        <div className={styles.header}>
          <h2 className={styles.sectionTitle}>🎯 HESI A2 Study Plans</h2>
          ...
          {plans.map((p, index) => (
            <article key={p.id} className={`${styles.card} ${p.popular ? styles.popular : ''} ${styles[`card${index % 3}`]}`}>
              ...
              <footer className={styles.cardFooter}>
                <button
                  type="button"
                  className={`${styles.action} ${p.popular ? styles.popularAction : ''} ${btnLoading[p.id] ? styles.loadingAction : ''}`}
                  onClick={() => handleGetStarted(p)}
                  disabled={!!btnLoading[p.id]}
                  aria-busy={!!btnLoading[p.id]}
                >
                  {btnLoading[p.id] ? (
                    <>
                      <span className={styles.buttonSpinner} aria-hidden>⏳</span>
                      <span className={styles.buttonText}> Redirecting…</span>
                    </>
                  ) : (
                    <>🚀 Get Started</>
                  )}
