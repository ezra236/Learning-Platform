...
export default function Format9({ question, mode, selectedAnswer, onAnswer, onBookmark, onReport }) {
  ...
  return (
    <div className={styles.container}>
      <section className={styles.passage}>
        {question.paragraph_html && <div dangerouslySetInnerHTML={{ __html: question.paragraph_html }} />}
      </section>

      {question.table_html && <section className={styles.table} dangerouslySetInnerHTML={{ __html: question.table_html }} />}
      ...
      <div className={styles.controls}>
        ...
        {mode === "reviewmode" && <button className={styles.reveal} onClick={() => setShowAnswer((s) => !s)}>{showAnswer ? "🙈 Hide" : "👁️ Show"}</button>}
      </div>
