...
export default function Frame({ mode, examname, displayExam }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (examRes.status === 200) {
        ...
        const attRes = await fetch(`${API_BASE}/api/hesiattempt/${data.id}/`, { credentials: "include" });
  ...
  const saveAttempt = async (payload) => {
    ...
    try {
      await fetch(`${API_BASE}/api/hesiattempt/${exam.id}/`, {
        method: "POST",
        credentials: "include",
        headers: {
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      const url = `${API_BASE}/api/hesiattempt/${exam ? exam.id : ""}/`;
      const data = {
        time_spent: used,
  ...
  const handleBookmark = async (questionId) => {
    try {
      const res = await fetch(`${API_BASE}/api/hesibookmark/`, {
        method: "POST",
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
  ...
  const handleReport = async (questionId, description) => {
    try {
      const res = await fetch(`${API_BASE}/api/hesireport/`, {
        method: "POST",
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
