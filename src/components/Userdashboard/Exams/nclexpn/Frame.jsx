...
export default function Frame({ mode, examname }) {
  ...
  useEffect(() => {
    async function init() {
      try {
        await fetchCsrf();
        const resp = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/prep/exams/by-name/?name=${encodeURIComponent(examname)}`, { credentials: 'include' });
        if (!resp.ok) throw new Error('Failed to load exam');
        const examJson = await resp.json();
        setExam(examJson);

        // check existing attempt
        const checkResp = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/prep/attempts/check/${examJson.id}/`, { credentials: 'include' });
        if (checkResp.ok) {
  ...
  async function startExam() {
    ...
    const p = (async () => {
      ...
      const res = await authFetch(`/prep/attempts/`, { method: 'POST', body: payload });
      // normalize result
  ...
  async function onStartFresh() {
    ...
    try {
      await authFetch(`/prep/attempts/${attemptRef.current.id}/`, { method: 'DELETE' });
  ...
  async function ensureAttemptAndPatch(payload, questionId) {
    ...
    try {
      await authFetch(`/prep/attempts/${at.id}/`, { method: 'PATCH', body: { selected_choices: selectedChoicesRef.current, last_question_order: currentIndex }});
      ...
      if (err && err.message && err.message.includes('404')) {
        try {
          const check = await authFetch(`/prep/attempts/check/${exam.id}/`);
          if (check.exists) {
            ...
            selectedChoicesRef.current = normalized.selected_choices || {};
            await authFetch(`/prep/attempts/${normalized.id}/`, { method: 'PATCH', body: { selected_choices: selectedChoicesRef.current, last_question_order: currentIndex }});
            lastSavedSnapshotRef.current = JSON.stringify(selectedChoicesRef.current);
          }
  ...
  async function flushPendingSave() {
    ...
    try {
      ...
      await authFetch(`/prep/attempts/${attemptRef.current.id}/`, { method: 'PATCH', body: { selected_choices: selectedChoicesRef.current, last_question_order: currentIndex }});
      const updated = { ...attemptRef.current, selected_choices: selectedChoicesRef.current, last_question_order: currentIndex };
      setAttempt(updated);
      attemptRef.current = updated;
  ...
  async function submitExam() {
    try {
      ...
      const res = await authFetch(`/prep/attempts/${attemptRef.current.id}/submit/`, { method: 'POST', body: payload });
        ...
        <div className={styles.leftControls}>
          ...
          <button 
            ...
            onClick={async () => {
              try {
                await flushPendingSave();
                await authFetch(`/prep/bookmarks/`, { method: 'POST', body: { question: curQ.id, exam: exam.id }});
